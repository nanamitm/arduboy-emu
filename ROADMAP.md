...
- [x] 実 ROM スモークテスト（ArduboyCollection 317 本を `examples/rom_smoke` で走査）。panic / 未知命令 / ロード失敗は 0、314 本が描画。ADC 変換完了漏れ（`analogRead` ポーリングが停止しないハング）を検出・修正
- [ ] 命令別セマンティクステストのさらなる拡充（サイクル数の網羅、LPM/ELPM 境界など）
- [ ] スモークテストの残存アウトライア（Ardletics: 低速/停滞 init）の根因調査

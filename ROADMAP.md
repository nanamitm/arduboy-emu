...
- [x] 実 ROM スモークテスト（ArduboyCollection 317 本を `examples/rom_smoke` で走査）。panic / 未知命令 / ロード失敗は 0、316 本が描画（残り 1 本 Chirp は micros() ベースの音声専用アプリで画面ブランクは仕様）。検出・修正したバグ: ①ADC 変換完了漏れ（`analogRead` ポーリングが停止しないハング）②Timer4 割り込みベクタアドレスの誤り（TOIE4 使用ゲームが __bad_interrupt→連続リセット）

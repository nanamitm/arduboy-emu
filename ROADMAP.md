...
- [x] CI 組込のゴールデン framebuffer 回帰テスト（`tests/rom_regression.rs`）。自作の最小 ROM が SSD1306 を駆動して決定的パターンを描画し、命令実行・SPI・ディスプレイ経路をまとめて FNV ハッシュで固定。外部/許諾 ROM 不要で `cargo test`（CI）に自動搭載
- [ ] 命令別セマンティクステストのさらなる拡充（サイクル数の網羅、LPM/ELPM 境界など）
- [ ] 回帰テストの拡充（複数パターン / Gamebuino PCD8544 経路のゴールデン化）

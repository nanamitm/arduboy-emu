...
- [x] 実 ROM スモークテスト（`examples/rom_smoke`、CPU 自動判定）。Arduboy 317 本は 316 本描画（残り 1 本 Chirp は micros() ベースの音声専用アプリで画面ブランクは仕様）、Gamebuino Classic 50 本は全描画。全 367 本で panic / 未知命令 / ロード失敗は 0。検出・修正したバグ: ①ADC 変換完了漏れ（`analogRead` ポーリングが停止しないハング）②Timer4 割り込みベクタアドレスの誤り（TOIE4 使用ゲームが __bad_interrupt→連続リセット）
- [ ] 命令別セマンティクステストのさらなる拡充（サイクル数の網羅、LPM/ELPM 境界など）
